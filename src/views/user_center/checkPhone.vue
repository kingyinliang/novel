<template>
    <div>
      <heads :tit="'修改手机'" :wechat="wechat"></heads>
      <div class="checkPhone">
        <ul>
            <li>
              <input type="text" v-model='checkphoneNumber' disabled placeholder='请输入您要绑定的手机号' class="phone">
            </li> 
            <li>
        ...
        <button id="register" class="goRegister login_button" @click="postForm">下一步</button>
      </div>
    </div>
</template>
<script>
  ...
  methods: {
    picCodeFun: function(index){
        ...
        httoProxy(`${HOME_API.USERINFO_API}`, 'GET', {}).then(res => {
            ...
            this.checkphoneNumber = res.data.phone.substr(0,3)+"****"+res.data.phone.substr(7); //手机
        })
    },
    ...
    postForm () {
        if(this.picCodeValue !='' && this.phoneCode !=''){
          httoProxy(`${HOME_API.CHECKPHONE_API}`, 'POST', {
            phone: this.checkphone ,
            captcha: this.picCodeValue, 
            mobileCode: this.phoneCode 
          }).then(res => { 
            if(res.data.code == 0){
               this.$router.push({path: '/modify_phone'})
            }else{
